...
parser.add_argument('--save', type=str,default='1501107698514463.pt',
...
from main import batchify, get_batch, repackage_hidden
...
def evaluate(data_source, batch_size=10, window=500):
    ...
    for i in range(0, data_source.size(0) - 1, args.bptt):
        ...
        for idx, vocab_loss in enumerate(softmax_output_flat):
            ...
            if start_idx + idx > window:
                ...
                #print([r.sum().data[0] for r in valid_pointer_history])
                #print('Target:', rnn_out[idx].sum())
                logits = torch.mv(valid_pointer_history, rnn_out[idx])
                logits = logits / torch.norm(valid_pointer_history, 2, 1)  / torch.norm(rnn_out[idx]).expand_as(logits)
                #print(logits)
                theta = args.theta
                #h_size = output_flat.size(1)
                ptr_attn = torch.nn.functional.softmax(theta * logits).view(-1, 1)
                #print(ptr_attn)
                #print(ptr_attn.size(), valid_next_word.size(), ptr_attn.expand_as(valid_next_word).size())
                ptr_dist = (ptr_attn.expand_as(valid_next_word) * valid_next_word).sum(0).squeeze()
                lambdah = args.lambdasm #0.1 # 0.2 # 0.25 # 0.1 # 0.25 # 0.1 # 0.3
